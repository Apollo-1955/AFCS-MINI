...
class AltitudeEstimator:
    ...
    def update(self):
        ...
        # Let upward continue, fall only if net_acc_z is negative
        if net_acc_z < 0:
        ...
        self.last_acc_z = net_acc_z
        return self.kalman.update(self.altitude), net_acc_z
...
def main():
    ...
    with open(filename, "w") as f:
        f.write("Time(s),Altitude(m),Z-Accel(m/s^2)\n")
    ...
    while True:
        try:
            altitude, z_accel = estimator.update()
            current_time = ticks_diff(ticks_ms(), start_time) / 1000.0

            print("Altitude: {:.2f} m | Z-Accel: {:.3f} m/s²".format(altitude, z_accel))

            # Add data to buffer
            log_buffer.append("{:.2f},{:.3f},{:.3f}\n".format(current_time, altitude, z_accel))

            # Save every 25 seconds
