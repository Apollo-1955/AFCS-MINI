    ...
    def update(self):
        ...
        if acc_mag < free_fall_threshold:
            if net_acc_z < 0:
                ...
                net_acc_z -= fake_force_strength
            self.velocity *= 0.7  # heavy damping during free fall
        else:
            if net_acc_z > 0:
                self.velocity *= 1.0  # no damping upward
            else:
                self.velocity *= 0.95  # light damping downward

        # Deadzone for small *negative* acceleration (avoid false falling)
        neg_deadzone = -0.08
        if net_acc_z < 0 and net_acc_z > neg_deadzone:
            net_acc_z = 0.0

        # Integrate velocity and altitude
        self.velocity += net_acc_z * dt

        # Clamp velocity to realistic values
        max_velocity = 50.0
...
# Find next file to avoid overwrite
...
def main():
    ...
    while True:
        try:
            alt = estimator.update()
            t = ticks_diff(ticks_ms(), start_time) / 1000.0
            print("Altitude: {:.2f} m".format(alt))
            log_buffer.append("{:.2f},{:.2f}\n".format(t, alt))

            # Save every 10 seconds
            if ticks_diff(ticks_ms(), last_write) >= 10000:
