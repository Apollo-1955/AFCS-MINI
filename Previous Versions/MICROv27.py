...
class KalmanFilter:
    def __init__(self, Q=0.05, R=0.3):  # tuned Q,R for smoother filtering
        self.Q = Q
        self.R = R
        self.P = 1.0
        self.X = 0.0
...
class AltitudeEstimator:
    ...
    def update(self):
        ...
        # Fake force for free-fall detection
        ...
        # Deadzone to ignore small fluctuations
        acc_deadzone_positive = 0.07  # increase positive deadzone to be less sensitive when going up
        acc_deadzone_negative = 0.03  # smaller deadzone for downward movements

        if net_acc_z > 0 and net_acc_z < acc_deadzone_positive:
            net_acc_z = 0.0
        elif net_acc_z < 0 and abs(net_acc_z) < acc_deadzone_negative:
            net_acc_z = 0.0
